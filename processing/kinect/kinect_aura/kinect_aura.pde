...
void setup() {
  ...
  aura = new PVector[auraNum];
  for(int i = 0; i < auraNum; i++) {
    aura[i] = new PVector(0, 0, 0);
  }
}
...
void drawAura() {
  ...
  aura[0].z = auraColor;
  for(int i = auraNum - 1; i >= 0; i--) {
    if(user == null) {                        // userが空なら
      return;                                 // 何もしないで関数を抜ける
    }
    fill(aura[i].z, 255, 255, 10 - int(10/auraNum)*i);
    ellipse(aura[i].x, aura[i].y,
